...
console.log('-----------')
let obj15 = { name: 'jspang', skill: 'web' };
let age15 = Symbol();
obj15[age15] = 18;
for (let item in obj15) {
    console.log(item, obj15[item]);
}
console.log(obj15);

console.log('Set和Array 的区别是Set不允许内部有重复的值，如果有只显示一个，相当于去重。虽然Set很像数组，但是他不是数组。');
let setArr = new Set(['a', 'b2', 'c']);
console.log(setArr);
setArr.add('前端职场');
console.log(setArr)
for (let [index, val] of setArr.entries()) {
    console.log(index, val);
}
console.log(setArr.has('a'));
console.log(setArr.size);
setArr.forEach((val, index) => console.log(val, index));
setArr.clear();
console.log(setArr);

let weakObj = new WeakSet();
let objw = { a: 'jspang', b: '技术胖' }
let objw2 = { a: 'jspang2', b: '技术胖2' }
let obj1w = objw;
